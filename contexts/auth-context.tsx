        ...
        photoGroups: [
          {
            ...
            photos: [
              ...
              { id: 2, src: "/placeholder.svg?height=600&width=600", alt: "Landscape photo", caption: "Forest path" },
              { id: 3, src: "/placeholder.svg?height=600&width=600", alt: "Landscape photo", caption: "Beach sunset" },
            ],
          },
          {
            id: "group-2",
          ...
          },
          {
            ...
            photos: [
              { id: 6, src: "/placeholder.svg?height=600&width=600", alt: "Urban photo", caption: "Urban exploration" },
              {
                id: 7,
                src: "/placeholder.svg?height=600&width=600",
        ...
        photoGroups: [
          {
            ...
            photos: [
              ...
              { id: 2, src: "/placeholder.svg?height=600&width=600", alt: "Wildlife photo", caption: "Elephant herd" },
            ],
          },
          {
            id: "group-2",
...
const MOCK_PRICING_PLANS: PricingPlan[] = [
  ...
  },
  {
    ...
    price: 12,
    billingCycle: "monthly",
    features: ["Unlimited photo groups", "Advanced analytics", "Custom domain", "Priority support"],
    isPopular: true,
    maxPhotoGroups: null,
    customDomain: true,
  ...
  },
  {
    ...
    price: 29,
    billingCycle: "monthly",
    features: ["Multiple portfolios", "Team collaboration", "Client proofing", "White labeling"],
    isPopular: false,
    maxPhotoGroups: null,
    customDomain: true,
    advancedAnalytics: true,
...
type AuthContextType = {
  ...
  updateGroupGridSettings: (groupId: string, settings: Partial<GridSettings>) => void
  // ... existing properties
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedPortfolioId = localStorage.getItem("photofolio_active_portfolio")
    ...
    if (savedUser) {
      ...
      if (savedPortfolioId && parsedUser.portfolios.length > 0) {
        const portfolio = parsedUser.portfolios.find((p) => p.id === savedPortfolioId) || parsedUser.portfolios[0]
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    if (foundUser) {
      ...
      if (foundUser.portfolios.length > 0) {
        ...
        localStorage.setItem("photofolio_active_portfolio", foundUser.portfolios[0].id)
      }
  ...
  const updatePortfolio = (portfolioData: Partial<Portfolio>) => {
    ...
    const updatedPortfolios = user.portfolios.map((p) => (p.id === activePortfolio.id ? updatedPortfolio : p))
  ...
  const updateUsername = async (newUsername: string): Promise<boolean> => {
    ...
    const isUsernameTaken = MOCK_USERS.some((u) => u.username === newUsername && u.id !== user.id)
  ...
  const deletePhotoGroup = (groupId: string) => {
    ...
    const updatedPhotoGroups = activePortfolio.photoGroups.filter((group) => group.id !== groupId)
    updatePortfolio({ photoGroups: updatedPhotoGroups })
  }
  ...
  const removePhotoFromGroup = (groupId: string, photoId: number) => {
    ...
    const updatedPhotoGroups = activePortfolio.photoGroups.map((group) => {
      if (group.id === groupId) {
        const updatedPhotos = group.photos.filter((photo) => photo.id !== photoId)

        // Update cover if the removed photo was the cover
        let updatedCoverId = group.coverId
        if (group.coverId === photoId) {
          updatedCoverId = updatedPhotos.length > 0 ? updatedPhotos[0].id : null
        }
  ...
  const updateGroupGridSettings = (groupId: string, settings: Partial<GridSettings>) => {
    if (!user || !activePortfolio) return
...
export type { PhotoGroup, Photo, GridSettings, UserRole, PricingPlan, Transaction, Subscription }
